...
class StoreMenu Extends Menu {
    ...
    prestigeArea := New PrestigeArea([
        , New Button(1235, 250, this.buttonMaxColor, this.buttonNotActiveColor, True, "Get Shares")
        , New SearchArea(2495, 100, 2525, 195, "*50 %A_ScriptDir%\..\application\game\scratch_inc\StoreMenuPrestige_2.png)", "Image")
        , True])
...
class SearchArea {
    __New(x1, y1, x2, y2, searchItem, searchType) {
        this.x1 := x1
        this.y1 := y1
        this.x2 := x2
        this.y2 := y2
    ...
    checkActive() {
        if (searchType = "Pixel") {
            PixelSearch, outX, outY, this.x1, this.y1, this.x2, this.y2, this.searchItem, , Fast RGB
        } else if (searchType = "Image") {
            ImageSearch, outX, outY, this.x1, this.y1, this.x2, this.y2, % this.searchItem
        } else {
            MsgBox, "Search Type Not Valid! Please provide a valid search type and reset the script."
            return False
        }
...
class PrestigeArea {
    __New(button, searchArea, active:=True) {
        this.button := button
    ...
    tryPrestige() {
        if (this.prestigeActive and this.searchArea.CheckActive()) {
            this.prestige()
            return True
        }
...
F5::scratch.playGame()
F6::
    PixelGetColor tempVar, 90, 485, RGB
    MsgBox, % tempVar
F9::Reload
